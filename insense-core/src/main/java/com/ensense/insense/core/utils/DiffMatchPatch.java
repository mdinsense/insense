...
public class DiffMatchPatch {
	public static Pattern DOLLAR_PATTERN = Pattern.compile("^\\$(([1-9]\\d{0,2}(,\\d{3})*)|(([1-9]\\d*)?\\d))(\\.\\d*)?");
  ...
  public int diff_commonPrefix(String text1, String text2) {
    ...
    for (int i = 0; i < n; i++) {
    	i = checkForSkipPattern(text1, text2, i);
    	i = Math.min(i, n-1);
  ...
  private int checkForSkipPattern(String text1, String text2, int i) {
	  int skip_index_for_text1 = 0;
	  int skip_index_for_text2 = 0;
	  
	  String subString = text1.substring(i, text1.length());
	  Matcher m = DOLLAR_PATTERN.matcher(subString);

	  if ( m.find() ){
		  skip_index_for_text1 = m.group().length();
	  }
	  
	  subString = text2.substring(i, text2.length());
	  m = DOLLAR_PATTERN.matcher(subString);
	  
	  if ( m.find() ){
		  skip_index_for_text2 = m.group().length();
	  }
	  
	  int n = Math.max(skip_index_for_text1, skip_index_for_text2);
	  i = i + n;
...
/**
  ...
  public String diff_prettyHtml(LinkedList<Diff> diffs) {
    ...
    for (Diff aDiff : diffs) {
      ...
      case INSERT:
        html.append("<ins style=\"background:#24941a;\">").append(text)
            .append("</ins>");
        break;
      case DELETE:
        html.append("<del style=\"background:#c90a0a;\">").append(text)
            .append("</del>");
        break;
      case EQUAL:
