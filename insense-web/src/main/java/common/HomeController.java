...
import com.cts.mint.common.model.BrokenReportSelection;
...
import com.cts.mint.common.utils.Constants.Menu;
import com.cts.mint.common.utils.Constants.ParentMenu;
...
import com.cts.mint.uitesting.model.FeedBackForm;
...
import com.cts.mint.uitesting.model.TransactionSummaryReport;
...
public class HomeController {
	...
	public String monitorJobs(RedirectAttributes redirAttr,
		...
		model.addAttribute("isShowUpdateJobStatus", UserServiceUtils.isUserHasAccessToFunctionality(httpRequest, Functionality.UPDATE_JOB_STATUS));
	...
	public ModelAndView Home(@ModelAttribute("model") ModelMap model, 
		...
		if(regressionTestExecutionForm.getSolutionType()<=0){
			regressionTestExecutionForm.setSolutionType(com.cts.mint.common.utils.Constants.SolutionType.UI_TESTING.getSolutionTypeId());
			model.addAttribute("solutionType", com.cts.mint.common.utils.Constants.SolutionType.UI_TESTING.getSolutionTypeId());
		...
		try {
			...
			if(UserServiceUtils.isUserHasAccessToFunctionality(httpRequest, Functionality.SHOW_ALL_UI_SUITS)) {
				uiSuitsCategoryList.add("Created By All");	
			}
			model.addAttribute("isManageSuitAccessAvailable",UserServiceUtils.isUserHasAccessToFunctionality(httpRequest, Functionality.MANAGE_SUITS));
			model.addAttribute("isMonitorJobAccessAvailable",UserServiceUtils.isUserHasAccessToFunctionality(httpRequest, Functionality.MONITOR_JOB_STATUS));
			
			//checking if mintv4 testsuits enabled
			boolean isMintV4SuitAccess = UserServiceUtils.isUserHasAccessToFunctionality(httpRequest, Functionality.MINTV4_SUIT);
			...
			model.addAttribute("solutionTypeList",
				...
				if(!model.containsAttribute("regressionTestSuit")) {
					if(regressionTestExecutionForm.getSolutionType() == com.cts.mint.common.utils.Constants.SolutionType.UI_TESTING.getSolutionTypeId()){
				...
				if(!model.containsAttribute("webserviceSuits")) {
					if(regressionTestExecutionForm.getSolutionType()== com.cts.mint.common.utils.Constants.SolutionType.WEBSERVICE.getSolutionTypeId()){
				...
				if(regressionTestExecutionForm.getUiSuitsCategory() !=null){
					if(regressionTestExecutionForm.getUiSuitsCategory().equals("Private Suits - "+currentUser.getUserName())) {
						...
						} else if(regressionTestExecutionForm.getSolutionType() == com.cts.mint.common.utils.Constants.SolutionType.WEBSERVICE.getSolutionTypeId()){
							model.addAttribute("webserviceSuits",
	...
	public String saveRegressionTestSuits(RedirectAttributes redirAttr,
		...
		String dateval = KEYS.BLANK;
		DateFormat formatter;
		DateFormat fromUser = null;
		String reccurencedateval = KEYS.BLANK;String stringTime = KEYS.BLANK;
		...
		try {
			...
			if (regressionTestExecutionForm.getScheduleType().equalsIgnoreCase(SCHEDULER.SCHEDULED_RUN)){
				...
				if (regressionTestExecutionForm.getExecuteBy().equals(SCHEDULER.WEEKLY_SCHEDULE)) {
					...
					ts.setScheduleType(SCHEDULER.BY_WEEKLY);//Weekwise
					
					String weekwise = KEYS.BLANK;
				...
				}	else if (regressionTestExecutionForm.getExecuteBy().equals(SCHEDULER.DATE_SCHEDULE)) {
					ts.setScheduleType(SCHEDULER.BY_DATE);//Datewise
	...
	public String uiChooseSuitType(RedirectAttributes redirAttr,
		...
		redirAttr.addFlashAttribute("suitTypes",
				menuService.getSubMenuList(ParentMenu.CONFIGURE_TEST));	
		logger.debug("Entry: UiChooseSuitType");
	...
	public String chooseSuitTypeSubmit(RedirectAttributes redirAttr,
		...
		try{
			...
			case Menu.BROKEN_LINK_CHECK:
				return "redirect:/BrokenLinkCheck.ftl";
				
			case Menu.FINDTEXTIMAGE:
				return "redirect:/UiFindImageOrText.ftl";
			default:
				return "redirect:/Home.ftl";
			}	
	...
	BrokenReportSelection getReportSelectionStatus(@RequestParam(value = "scheduleExecutionId") int scheduleExecutionId){
		...
		try{
			reportStatus.setAnalyticsReport(this.isReportSelectionExist(
					scheduleExecutionId,ReportPattern.ANALYTICS_REPORT_PATTERN.getReportPatternName()).equals("FileExist"));
			reportStatus.setAnalyticsErrorReport(this.isReportSelectionExist(
					scheduleExecutionId,ReportPattern.ANALYTICS_ERROR_REPORT_PATTERN.getReportPatternName()).equals("FileExist"));
			reportStatus.setBrokenLinkAvailable(this.isReportSelectionExist
					(scheduleExecutionId,ReportPattern.BROKEN_URL_REPORT_PATTERN.getReportPatternName()).equals("FileExist"));
			reportStatus.setBrokenLinkResourcesAvailable(this.isReportSelectionExist(
					scheduleExecutionId,ReportPattern.BROKEN_URL_WITH_RESOURCES_REPORT_PATTERN.getReportPatternName()).equals("FileExist"));
			reportStatus.setLoadTimeAttributeAvailable(this.isReportSelectionExist(
					scheduleExecutionId,ReportPattern.LOAD_TIME_ATTRIBUTES_REPROT_PATTERN.getReportPatternName()).equals("FileExist"));
			reportStatus.setComparisonReport(this.isReportSelectionExist(
					scheduleExecutionId,ReportPattern.REGRESSION_COMPARISON_REPORT_PATTERN.getReportPatternName()).equals("FileExist"));
			reportStatus.setImageTextReport(this.isReportSelectionExist(
					scheduleExecutionId,ReportPattern.IMAGE_TEXT_COMPARISON_REPORT_PATTERN.getReportPatternName()).equals("FileExist"));
			reportStatus.setImageTextBaslineReport(this.isReportSelectionExist(
					scheduleExecutionId,ReportPattern.IMAGE_TEXT_COMPARISON_REPORT_BASELINE_PATTERN.getReportPatternName()).equals("FileExist"));
			reportStatus.setAllImageZipFile(this.isReportSelectionExist(
					scheduleExecutionId,ReportPattern.NORMALIZED_ZIP.getReportPatternName()).equals("FileExist"));
	...
	private String downloadAllBrokenReportsLinks(String filePath,
		...
		List<File> files = (List<File>) org.apache.commons.io.FileUtils.listFiles(dir, extensions,
				true);
	...
	String isAnalyticsFailureReportExist(
		...
		try{
			...
			if(MintFileUtils.checkFileExists(reportsDirectoryPath, "MINT_Analytics_FAILURE_Report", FILE.XLSX)) {
				isExist = "FileExist";
			} 
	...
	String isAnalyticsReportExist(
		...
		try{
			...
			String reportsDirectoryPath = scheduleExecution.getReportsDirectory();
			logger.info("reportsDirectoryPath :" +reportsDirectoryPath);
			if(MintFileUtils.checkFileExists(reportsDirectoryPath, "MINT_Analytics_Report", FILE.XLSX)) {
				isExist = "FileExist";
			} 
	...
	public void DownloadAnalyticsFailureReport(
		...
		try{
			...
			if(MintFileUtils.checkFileExists(harReportsDirectoryPath, "MINT_ANALYTICS_ERROR_REPORT", FILE.XLSX)) {
				logger.info("DownloadAnalyticsFailureReport : Report exists" );
				new MintFileUtils().writeReportToStream(harReportsDirectoryPath, "MINT_ANALYTICS_ERROR_REPORT", response,servletContext, FILE.XLSX);
			} else {
	...
	public void DownloadAnalyticsReport(
		...
		try{
			...
			String harReportsDirectoryPath = scheduleExecution.getReportsDirectory();
			logger.info("harReportsDirectoryPath :" +harReportsDirectoryPath);
			if(MintFileUtils.checkFileExists(harReportsDirectoryPath, "MINT_ANALYTICS_REPORT", FILE.XLSX)) {
				logger.info("DownloadAnalyticsReport : Report exists" );
				new MintFileUtils().writeReportToStream(harReportsDirectoryPath, "MINT_ANALYTICS_REPORT", response,servletContext, FILE.XLSX);
			} else {
	...
	String isComparisonReportExist(
		...
		try{
			...
			String reportsDirectoryPath = scheduleExecution.getReportsDirectory();
			logger.info("reportsDirectoryPath :" +reportsDirectoryPath);
			if(MintFileUtils.checkFileExists(reportsDirectoryPath, "MINT_REGRESSION_COMPARISON_REPORT", FILE.XLSX)) {
				isExist = "FileExist";
			} 
	...
	public void DownloadComparisonReport(
		...
		try{
			...
			if(MintFileUtils.checkFileExists(reportsDirectoryPath, "MINT_REGRESSION_COMPARISON_REPORT", FILE.XLSX)) {
				...
				new MintFileUtils().writeReportToStream(reportsDirectoryPath, "MINT_REGRESSION_COMPARISON_REPORT", response,servletContext, FILE.XLSX);
			} else {
		...
		try {
			...
			if (regressionTestExecutionForm.getScheduleType().equalsIgnoreCase(SCHEDULER.SCHEDULED_RUN)){
				...
				if (regressionTestExecutionForm.getExecuteBy().equals(SCHEDULER.WEEKLY_SCHEDULE)) {
					...
					wsSchedule.setScheduleType(SCHEDULER.BY_WEEKLY);//Weekwise
					...
					wsSchedule.setScheduleType(SCHEDULER.BY_DATE);//Datewise
					reccurencedateval= regressionTestExecutionForm.getScheduledDate().substring(0,11) + " " + timeValue;
					Date nowDate=fromUser.parse(reccurencedateval);
...
*/
			...
			regressionTestExecutionForm.setSetupTabNumber(5);
			regressionTestExecutionForm.setSolutionType(com.cts.mint.common.utils.Constants.SolutionType.WEBSERVICE.getSolutionTypeId());
			model.addAttribute("solutionType", com.cts.mint.common.utils.Constants.SolutionType.WEBSERVICE.getSolutionTypeId());
			redirAttr.addFlashAttribute("regressionTestExecutionForm", regressionTestExecutionForm);
		} catch (Exception e) {
	...
	String isReportSelectionExist(
		...
		String reportType = FILE.XLSX;
		
		try{
			
			ScheduleExecution scheduleExecution = scheduledService.getScheduleExecution(scheduleExecutionId);
			String reportsDirectoryPath = scheduleExecution.getReportsDirectory();
			if(reportName.equals(ReportPattern.ANALYTICS_REPORT_PATTERN.getReportPatternName())){
				reportNameStartsWith = ReportPattern.ANALYTICS_REPORT_PATTERN.getReportPatternName();	
			}else if(reportName.equals(ReportPattern.ANALYTICS_ERROR_REPORT_PATTERN.getReportPatternName())){
				reportNameStartsWith = ReportPattern.ANALYTICS_ERROR_REPORT_PATTERN.getReportPatternName();
			}else if(reportName.equals(ReportPattern.BROKEN_URL_REPORT_PATTERN.getReportPatternName())){
				reportNameStartsWith = ReportPattern.BROKEN_URL_REPORT_PATTERN.getReportPatternName();
			}else if(reportName.equals(ReportPattern.BROKEN_URL_WITH_RESOURCES_REPORT_PATTERN.getReportPatternName())){
				reportNameStartsWith = ReportPattern.BROKEN_URL_WITH_RESOURCES_REPORT_PATTERN.getReportPatternName();
			}else if(reportName.equals(ReportPattern.LOAD_TIME_ATTRIBUTES_REPROT_PATTERN.getReportPatternName())){
				reportNameStartsWith = ReportPattern.LOAD_TIME_ATTRIBUTES_REPROT_PATTERN.getReportPatternName();
			}else if(reportName.equals(ReportPattern.REGRESSION_COMPARISON_REPORT_PATTERN.getReportPatternName())){
				reportNameStartsWith = ReportPattern.REGRESSION_COMPARISON_REPORT_PATTERN.getReportPatternName();
			}else if(reportName.equals(ReportPattern.IMAGE_TEXT_COMPARISON_REPORT_PATTERN.getReportPatternName())){
				reportNameStartsWith = ReportPattern.IMAGE_TEXT_COMPARISON_REPORT_PATTERN.getReportPatternName();
			} else if(reportName.equals(ReportPattern.IMAGE_TEXT_COMPARISON_REPORT_BASELINE_PATTERN.getReportPatternName())){
				reportNameStartsWith = ReportPattern.IMAGE_TEXT_COMPARISON_REPORT_BASELINE_PATTERN.getReportPatternName();
			}
			else if(reportName.equals(ReportPattern.NORMALIZED_ZIP.getReportPatternName())){
				//reportsDirectoryPath += com.cts.mint.common.utils.Constants.BrokenReportSelection.ALL_IMAGE_DIR;
				reportNameStartsWith = ReportPattern.NORMALIZED_ZIP.getReportPatternName();
				reportType = FILE.ZIP;
			}
	...
	public void DownloadReportSelection(
		...
		try{
			...
			if(reportPatternName == ReportPattern.ANALYTICS_REPORT_PATTERN.getReportPatternId()){
				reportPattern = ReportPattern.ANALYTICS_REPORT_PATTERN.getReportPatternName();	
			}else if(reportPatternName == ReportPattern.ANALYTICS_ERROR_REPORT_PATTERN.getReportPatternId()){
				reportPattern = ReportPattern.ANALYTICS_ERROR_REPORT_PATTERN.getReportPatternName();
			}else if(reportPatternName == ReportPattern.BROKEN_URL_REPORT_PATTERN.getReportPatternId()){
				reportPattern = ReportPattern.BROKEN_URL_REPORT_PATTERN.getReportPatternName();
			}else if(reportPatternName == ReportPattern.BROKEN_URL_WITH_RESOURCES_REPORT_PATTERN.getReportPatternId()){
				reportPattern = ReportPattern.BROKEN_URL_WITH_RESOURCES_REPORT_PATTERN.getReportPatternName();
			}else if(reportPatternName == ReportPattern.LOAD_TIME_ATTRIBUTES_REPROT_PATTERN.getReportPatternId()){
				reportPattern = ReportPattern.LOAD_TIME_ATTRIBUTES_REPROT_PATTERN.getReportPatternName();
			}else if(reportPatternName == ReportPattern.REGRESSION_COMPARISON_REPORT_PATTERN.getReportPatternId()){
				reportPattern = ReportPattern.REGRESSION_COMPARISON_REPORT_PATTERN.getReportPatternName();
			}else if(reportPatternName == ReportPattern.IMAGE_TEXT_COMPARISON_REPORT_PATTERN.getReportPatternId()){
				reportPattern = ReportPattern.IMAGE_TEXT_COMPARISON_REPORT_PATTERN.getReportPatternName();
			}else if(reportPatternName == ReportPattern.IMAGE_TEXT_COMPARISON_REPORT_BASELINE_PATTERN.getReportPatternId()){
				reportPattern = ReportPattern.IMAGE_TEXT_COMPARISON_REPORT_BASELINE_PATTERN.getReportPatternName();
			} else if(reportPatternName == ReportPattern.NORMALIZED_ZIP.getReportPatternId()){
				reportPattern = ReportPattern.NORMALIZED_ZIP.getReportPatternName();
