 ...
 * urlTheme
 *
 * Macro to translate a theme message a url that is context specific.
 -->
<#macro urlTheme code>${springMacroRequestContext.getContextPath()}${springMacroRequestContext.getThemeMessage(code)}</#macro>
...
<#macro formInput path attributes="" fieldType="text">
    <@bind path/>
    <input type="${fieldType}" id="${status.expression}" name="${status.expression}" value="<#if fieldType!="password"><#if stringStatusValue?exists >${stringStatusValue?html}</#if></#if>" ${attributes}<@closeTag/>
</#macro>

<#macro formInput1 id path attributes="" fieldType="hidden">
    <@bind path/>
    <input type="${fieldType}" id="${id}" name="${status.expression}" value="<#if fieldType!="password"><#if stringStatusValue?exists >${stringStatusValue?html}</#if></#if>" ${attributes}<@closeTag/>
</#macro>

<#macro formInputMoney path attributes="" fieldType="text">
    <@bind path/>
    <input type="${fieldType}" id="${status.expression}" name="${status.expression}" value="<#if status.value?exists && status.value?is_number>${status.value?string("0.00")}<#else></#if>" ${attributes}<@closeTag/>
</#macro>
...
<#macro formHiddenInput1 id path attributes="">
    <@formInput1 id path, attributes, "hidden"/>
</#macro>
...
<#macro formTextarea path attributes="">
    <@bind path/>
    <textarea id="${status.expression}" name="${status.expression}" ${attributes}><#if stringStatusValue?exists >${stringStatusValue?html}</#if></textarea>
</#macro>
...
<#macro formSingleSelect path options attributes="">
    ...
    <select id="${status.expression}" name="${status.expression}" ${attributes}>
	    ...
	    <#elseif options?is_enumerable>
	        <#list options as entry>
	        	<option value="${entry.key?html}"<@checkSelected entry.key/>>${entry.value?html}</option>
	        </#list>
	    </#if>
    </select>
</#macro>
<#macro formSingleSelect2 id path options attributes="">
    <@bind path/>
    <select id="${id}" name="${status.expression}" ${attributes}>
    		<#list options?values as item>
				<option value="${item?html}">${item.displayValue?html}</option>
			</#list> 
    </select>
</#macro>

<#macro formSingleSelectEnumMap id path options attributes="">
    <@bind path/>
    <select id="${id}" name="${status.expression}" ${attributes}>
...
<#macro singleSelectDisplayValueOnly path options>
    <@bind path/>
	<#if options?is_hash>
        <#list options?keys as value>
		    <#if stringStatusValue?is_number && stringStatusValue == value?number>${options[value]?html}</#if>
		    <#if entry.key?exists && entry.key != ''>
		    	<#if stringStatusValue?is_string && stringStatusValue == value>${options[value]?html}</#if>
		    </#if>
        </#list>
    <#elseif options?is_enumerable>
        <#list options as entry>
		    <#if stringStatusValue?is_number && stringStatusValue == entry.key?number>${entry.value?html}</#if>
		    <#if entry.key?exists && entry.key != ''>
		    	<#if stringStatusValue?is_string && stringStatusValue == entry.key>${entry.value?html}</#if>
		    </#if>
        </#list>
    </#if>
</#macro>
 ...
 *    or CSS styles or size
-->
<#macro formSelectBool path allowEmpty=false attributes="">
    <@bind path/>
    <select id="${status.expression}" name="${status.expression}" ${attributes}>
    	<#if allowEmpty><option value=""<#if !status.value?exists> selected="selected"</#if>></option></#if>
        <option value="true"<#if status.value?exists && "${status.value}" = "true"> selected="selected"</#if>><@spring.message "text.yes"/></option>
        <option value="false"<#if (!allowEmpty && !status.value?exists) || (status.value?exists && "${status.value}" != "true")> selected="selected"</#if>><@spring.message "text.no"/></option>
    </select>
</#macro>
...
<#macro formMultiSelect path options attributes="">
    ...
    <select multiple="multiple" id="${status.expression}" name="${status.expression}" ${attributes}>
	    ...
	    <#elseif options?is_enumerable>
	        <#list options as entry>
	        	<#assign isSelected = contains(status.actualValue?default([""]), entry.key)>
	        	<option value="${entry.key?html}"<#if isSelected> selected="selected"</#if>>${entry.value?html}</option>
	        </#list>
	    </#if>
    </select>
</#macro>
<#macro formMultiSelect3 id path options attributes="">
    <@bind path/>
    <select multiple="multiple" id="${id}" name="${status.expression}" ${attributes}>
		<#list options as item>
			<option value="${item.code?html}">${item.value?html}</option>
		</#list> 
    </select>
</#macro>
 ...
 *    will be used.
-->
<#macro showErrors separator="<br />" classOrStyle="error">
	<#if ( status.errorMessages?exists && status.errorMessages?is_sequence && status.errorMessages?size > 0 ) >
		<#if classOrStyle?index_of(":") == -1><#assign attr="class"><#else><#assign attr="style"></#if>
		<div ${attr}="${classOrStyle}">
...
-->
<#function contains ls item>
    <#list ls as nextInList>
