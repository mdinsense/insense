...
<div id="Ui AnalyticsTesting Setup">
...
function enableSmartUser(){
	if(document.aform.smartUser.checked == true){
		document.aform.smartUser.value = "on";
	}else{
		document.aform.smartUser.value = "off";
	}
}


function enableScrollPage(){
	if(document.aform.scrollPage.checked == true){
		document.aform.smartUser.value = "on";
	}else{
		document.aform.scrollPage.value = "off";
	}
}
...
function validateAndsave() {
	...
	if(document.aform.suitId.value != "" && document.aform.suitId.value > 0) {
		document.getElementById("analyticsTestingId").setAttribute('action',"<@spring.url '/UpdateAnalyticsTesting.ftl' />");
	} else {
		document.getElementById("analyticsTestingId").setAttribute('action',"<@spring.url '/SaveAnalyticsTesting.ftl' />");
	}
	
	document.getElementById("analyticsTestingId").submit();
}
...
<div class="page-content" id="pagecontent" role="main" tabindex="-1">
			...
			<div class="mblg">
				<div class="mtxs">
					<a id="tcId100" class="tipLink rel" name="tcId100">Help</a><b><@spring.message
						"mint.help.analytics"/></b><a href="<@spring.message 'mint.help.mailTo'/>"
        ...
        <form  modelAttribute="testingForm" action='SaveAnalyticsTesting.ftl' id="analyticsTestingId" name="aform" target="_top" method="POST">
					...
					<tr class="bd">
						<td style="padding:10px">
								...
								<table class="bd rowheight35">
									...
									<tr class="lblFieldPair">
										<td class="lbl">Smart User</td>
										<td class="input">
											<#if testingForm?exists && testingForm.smartUser?exists && testingForm.smartUser== true>
												<input type="checkbox" id="smartUser" name="smartUser" checked="checked">
											<#elseif testingForm?exists && testingForm.smartUser?exists && testingForm.smartUser== false>
												<input type="checkbox" id="smartUser" name="smartUser" onclick="enableSmartUser()">
											<#else>
												<input type="checkbox" id="smartUser" name="smartUser" checked="checked" onclick="enableSmartUser()">
											</#if>	
										</td>
									</tr>
									<tr class="lblFieldPair">
										<td class="lbl">Wait Time</td>
										<td class="input">
												<input type="text" id="waitTime" name="waitTime" checked="checked" value="${waitTime}" >
										</td>
									</tr>
									<tr class="lblFieldPair">
										<td class="lbl">Scroll Page</td>
										<td class="input">
											<#if testingForm?exists && testingForm.scrollPage?exists && testingForm.scrollPage>
												<input type="checkbox" id="scrollPage" name="scrollPage" checked="checked">
											<#elseif testingForm?exists && testingForm.scrollPage?exists && !testingForm.scrollPage>
												<input type="checkbox" id=scrollPage name="scrollPage" onclick="enableScrollPage()">
											<#else>
												<input type="checkbox" id="scrollPage" name="scrollPage" checked="checked" onclick="enableScrollPage()">
											</#if>	
										</td>
									</tr>
								</table>
